...
因此终端 20 行图表具有约 40 层纵向分辨率。鼠标坐标映射到采样索引，只有跨越采样点时才更新历史选择。键盘或鼠标选择历史后，图表在上下边框绘制位置箭头，只在数据上方的空白区域补充细虚线，不覆盖堆叠图元，避免游标被误认为采集断层。右侧应用列表读取同一快照并在标题显示采样时间。历史快照只包含应用汇总，因此 PID 面板会明确提示历史不保存 PID；回到最新时刻后重新显示当前 PID。
