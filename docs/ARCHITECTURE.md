...
历史同时受 `retention` 和 `maximum_samples` 限制。TUI 按 `retention / 图表宽度` 生成与 Unix 时间对齐的固定时间桶，每桶保留最后一个点并保留真实空桶。新采样位于当前桶时只更新当前列，跨桶时历史只左移一列，不会因样本总数变化重新选择整段历史。
...
因此终端 20 行图表具有约 40 层纵向分辨率。前 N 应用从完整可见历史计算，并使用会话内固定系列槽位；排名互换时颜色和堆叠层保持不变，新应用只接替退出应用的槽位。“其他”始终使用最后一种颜色。纵轴按完整历史峰值取细粒度稳定档位，增长立即生效，下降需连续多个新样本确认且每次最多退一档。鼠标、左右键和历史游标使用同一时间桶映射。键盘或鼠标选择历史后，图表在上下边框绘制位置箭头，只在数据上方的空白区域补充细虚线，不覆盖堆叠图元。右侧应用列表读取同一快照并在标题显示采样时间。历史快照只包含应用汇总，因此 PID 面板会明确提示历史不保存 PID；回到最新时刻后重新显示当前 PID。
